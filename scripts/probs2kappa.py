...
from scripts.util import quadratic_weighted_kappa
...
def load_softmax_outputs():
    ...
    with open('data/processed/1234ce0_jfnet_100MCdropout_KaggleDR_train.pkl', 'rb') as h:
    ...
    with open('data/processed/40d8265_jfnet_100MCdropout_KaggleDR_test.pkl', 'rb') as h:
...
def main(compute_optimal_thresholds=True):
    ...
    print('Opt. train kappa:',
          quadratic_weighted_kappa(y_train, probs_to_labels(softmax_train, thr), 5))
    print('Opt. test kappa:',
          quadratic_weighted_kappa(y_test, probs_to_labels(softmax_test, thr), 5))

    print('Argmax train kappa:',
          quadratic_weighted_kappa(y_train, np.argmax(softmax_train, axis=1), 5))
    print('Argmax test kappa:',
          quadratic_weighted_kappa(y_test, np.argmax(softmax_test, axis=1), 5))

    private = (df_test.Usage == 'Private').values
    public = (df_test.Usage == 'Public').values
    print('Opt. test (private leaderboard) kappa:',
          quadratic_weighted_kappa(y_test[private], probs_to_labels(softmax_test[private], thr), 5))
    print('Opt. test (public leaderboard) kappa:',
          quadratic_weighted_kappa(y_test[public], probs_to_labels(softmax_test[public], thr), 5))
