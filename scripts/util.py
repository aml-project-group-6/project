...
class Progplot(object):
    ...
    def save(self, filename):
        self.output_file = bp.output_file(filename, title='Progression plot')
...
class TrainingMonitor(tf_keras.callbacks.Callback):
...
class AdaptiveLearningRateScheduler(tf_keras.callbacks.Callback):
...
class SelectiveSampler(object):
    ...
    def _random_sample(self, case='neg'):
        if case == 'pos':
            selection = np.random.randint(low=0, high=len(self.Xpos), size=self.M)
            return self.Xpos[selection]
        if case == 'neg':
            selection = np.random.randint(low=0, high=len(self.Xneg), size=self.M)
...
def roc_curve_plot(y_true, y_score, pos_label=1,
    ...
    fdr_low, tdr_low, _ = roc_curve(y_true[low.index], y_score[low.index], pos_label=pos_label)
    fdr_high, tdr_high, _ = roc_curve(y_true[high.index], y_score[high.index], pos_label=pos_label)
    ...
    plt.plot([0, 1], [0, 1], 'k--')
    if recommendation:
        plt.scatter([0.05], [0.8], color='g', s=50, label='recommendation BDA')
        plt.scatter([0.2], [0.85], color='b', s=50, label='recommendation NHS')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.0])
